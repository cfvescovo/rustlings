...
impl TryFrom<&str> for Person {
    ...
    fn try_from(s: &str) -> Result<Self, Self::Error> {
        ...
        if name_age.len() != 2 {
            Err(String::from("Bad string provided"))
        } else {
            let name = String::from(name_age[0]);
            let age: Result<usize, _> = name_age[1].parse();
            match age {
                Ok(usize_age) => Ok(Person{
                    name: name,
                    age: age.unwrap()
                }),
                Err(error) => Err(String::from("Age is not valid"))
            }
        }
    }
}
